...
async function insert(req, res) {
  ...
  try {
    if (!hallBookings || !Array.isArray(hallBookings) || hallBookings.length === 0) {
      throw new Error("hallBookings must be a non-empty array.");
    }

    const result = await sequelize.transaction(async (t) => {
      const { currentValue: bookingVal, prefix: bookingPrefix } =
        (await baseRepo.getCurrentSequence(constants.SEQUENCE_NAMES.HALL_BOOKING)) || {};
    ...
    if (error.name === "SequelizeUniqueConstraintError") {
      sendError(
        res,
        { ...error, message: "One or more halls are already booked for the given slot and date." },
